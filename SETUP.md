...
crack-bot attaches to the Bot's Chrome when it exposes a DevTools endpoint. You get the Bot's cookies and logins, the tab is brought to front before each click so the Bot can watch it, and on `blocked` screenshot computer use continues on the same tab.
...
3. Start `fast_web_task`. Chrome shows a permission dialog the first time crack-bot connects; the Bot clicks **Allow**. crack-bot keeps one connection open across tasks, so this happens once per Chrome session.

Leave `CDP_URL` unset. crack-bot finds Chrome browser processes in `ps`, reads each one's `DISPLAY` from `/proc/<pid>/environ`, and keeps only those on **this agent's** `DISPLAY` (the one the MCP server inherits, which is the Computer view). From those it reads `DevToolsActivePort`, confirms the port is listening, and connects to `ws://127.0.0.1:<port><path>`. On a multi-agent box with many `chrome-profile-*` on `:5`, `:9`, `:14`, … it will only ever attach to the Chrome on your display. If no Chrome on this display exposes DevTools, crack-bot does **not** fall back to another agent's Chrome; it launches its own headed Chromium on this `DISPLAY` so the page still shows in your Computer view.

The result reports `attached`, `attachedTo` (profile dir) and `display` so you can confirm which Chrome is being driven.

Overrides:

- `CRACK_BOT_CDP_PROFILE_DIR=/home/bot/chrome-profile-14` pins one profile; crack-bot errors if it has no live `DevToolsActivePort`.
- `CRACK_BOT_CDP_DISPLAY=:14` scopes discovery to a display other than the inherited `DISPLAY`.
...
- `CRACK_BOT_CDP_DISCOVER=false` always launches crack-bot's own Chromium.
...
When attached, `fast_web_abort` closes only the crack-bot tab and never quits Chrome. Finished runs leave the tab open for handoff (`open: true`); a `done` run in crack-bot's own Chromium closes it. Override with `keepOpen`.
...
For multi-step goals, each step also offers up to `CRACK_BOT_PLAN_TOP_K` (32) offscreen links ranked against the goal; picking one scrolls to it and clicks. Jev acts alone when its pick is clear (leads the runner-up by `CRACK_BOT_ASK_MARGIN`, 0.2). Independent `goal_done` / `stuck` watchers ride along with every action pick; `goal_done` ≥ 0.85 finishes the run, `stuck` ≥ 0.85 after step 2 is treated like BLOCKED. A repeated no-op takes the runner-up instead of retrying. When Jev says BLOCKED, is torn, or is about to click something irreversible, the run pauses with `status: "need_decision"` and the Bot chooses with `fast_web_choose`. The result's `visited`, `watchers`, `pageErrors`, and `stats` explain what happened.
...
> Add a custom MCP server named `crack-bot`. Command: `npx`. Args: `--yes tsx ./src/index.ts`. Working directory: this repo. Env: `AI_GATEWAY_API_KEY` from my saved secret.
...
- `attached` is false but you enabled remote debugging — check that `DevToolsActivePort` exists in the Bot's Chrome profile and that the Bot clicked **Allow** on Chrome's permission dialog. On Linux also confirm `DISPLAY` in the MCP server's environment matches the Chrome's (`cat /proc/<chrome pid>/environ | tr '\0' '\n' | grep DISPLAY`); if the server was started without `DISPLAY`, set `CRACK_BOT_CDP_DISPLAY`.
- `attached` is true but the page appears in another agent's Computer view — the result's `attachedTo` / `display` show which Chrome was chosen. Set `CRACK_BOT_CDP_PROFILE_DIR` to your profile, or make sure `DISPLAY` is set for the server.
